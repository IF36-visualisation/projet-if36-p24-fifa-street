...
### Le nombre de blessures est-il lié au nombre de match joué ou est-ce que le championnat joue une plus grosse partie . 
...
### Quel est le profil de buteur le plus prolifique  ?
...
### Déterminer le profil de l'équipe parfaite  **
...
### Quelle est la corrélation entre la valeur marchande des joueurs et leurs notes / postes ? 
...
### Comment les données on été traitées ?

Cette partie de la réponse s'intéresse à la manière dont les données ont été traitées avant la visualisation, il est donc préférable de consulter le code en parallèle du rapport pour mieux comprendre.
...
Avant de faire des visualisations, on va ordonner les lignes en fonction des positions des 5 premières équipes dans le classement de la ligue, à savoir Manchester City, Liverpool, Chelsea, Tottenham, et Arsenal.
...
On va rajouter l'équipe de Southampton, qui est 15ème de la ligue, et donc une équipe de bas niveau,et va également rajouter l'équipe de Newcastle, qui est 11ème de la ligue, et donc une équipe de milieu de tableau.
...
### Visualisation du nombre de buts par équipe
...
```{r}	
big5_data_by_team <- big5_data_by_team %>% mutate(Equipe = factor(Equipe, levels = rev(c("Manchester City","Liverpool","Chelsea","Tottenham","Arsenal","Manchester Utd" ,"West Ham","Leicester City","Brighton","Wolves","Newcastle Utd","Crystal Palace","Brentford","Aston Villa","Southampton","Everton","Leeds United","Burnley","Watford","Norwich City"))))


ggplot(big5_data_by_team, aes(x = Touches, y = Equipe)) +
  geom_bar(stat = "identity", fill = "#F36143") +
  theme_minimal() +
  labs(title = "Nombre de touches par équipe", x = "Nombre de touches", y = "Equipe") +
  theme(axis.text.y = element_text(angle = 0, vjust = 0.5, hjust = 1)) +
  coord_cartesian(xlim = c(40000,50000))


big5_data_by_team <- big5_data_by_team %>% mutate(Equipe = factor(Equipe, levels = c("Manchester City","Liverpool","Chelsea","Tottenham","Arsenal","Manchester Utd" ,"West Ham","Leicester City","Brighton","Wolves","Newcastle Utd","Crystal Palace","Brentford","Aston Villa","Southampton","Everton","Leeds United","Burnley","Watford","Norwich City")))

# Ci-dessous, en commentaire, le code pour les graphes sur le %age de pressing et de dribble réussis, non affichés car résultats non concluants

#ggplot(big5_data_by_team, aes(x = Equipe, y = Reussite_pressing, fill = Equipe)) + geom_bar(stat = "identity") + theme_minimal() + labs(title = "Pourcentage de pressing réussis par équipe", x = "Equipe", y = "Pourcentage de pressing réussis")+  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+guides(fill = guide_legend(ncol = 2))

#ggplot(big5_data_by_team, aes(x = Equipe, y = Reussite_Dribble, fill = Equipe)) + geom_bar(stat = "identity") + theme_minimal() + labs(title = "Pourcentage de dribbles réussis par équipe", x = "Equipe", y = "Pourcentage de dribbles réussis")+  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+ guides(fill = guide_legend(ncol = 2))


```

Sur ce graphe, l'équipe ayant obtenu la première place est en haut, et celle ayant obtenu la dernière place est en bas. On observe alors un résultat très concluant lorsque l'on analyse le nombre de touche, et on constate que le placement de l'équipe semble être correlée à au classement de l'équipe 

### Et pour d'autre paramètres ?

On a pu tester d'autres paramètres comme par exemple le pourcentage de réussite de pressing ou de dribble, mais également la somme de dribbles ou de pressing réussis, et on constate qu'il n'y a finalement que très peu de différence entre les équipes pour ces paramètres. On peut donc en conclure que ces paramètres ne sont pas déterminants pour différencier les équipes entre elles. Cela peut s'expliquer par le fait que les équipes de haut niveau ont des joueurs de qualité, et que ces paramètres sont donc assez homogènes entre les équipes de haut niveau.

On constate finalement que deux paramètres semblent être lié au classement des équipes dans la ligue : le nombre de but, et le nombre de touches.  

```{r, echo=FALSE, eval=FALSE, warning=FALSE,message=FALSE}
#Ici, le code pour le nombre de dribble et pressing réussi, non executé pour le rapport
big5_data_by_team <- read_csv("./data/dataset/big5_data_by_teams/big5_data_by_team.csv",show_col_types = FALSE)
big5_data_by_team <- big5_data_by_team %>% filter(Season_End_Year == 2022 & Squad %in% c("Manchester City","Liverpool","Chelsea","Tottenham","Arsenal","Manchester Utd" ,"West Ham","Leicester City","Brighton","Wolves","Newcastle Utd","Crystal Palace","Brentford","Aston Villa","Southampton","Everton","Leeds United","Burnley","Watford","Norwich City"
))
...
big5_data_by_team <- big5_data_by_team %>% mutate(Equipe = factor(Equipe, levels = c("Bordeaux", "Metz", "Saint-Étienne", "Clermont Foot", "Lorient", "Troyes", "Angers", "Montpellier", "Reims", "Brest", "Lille", "Nantes", "Lyon", "Lens", "Strasbourg", "Nice", "Rennes", "Monaco", "Marseille", "Paris S-G")))

ggplot(big5_data_by_team, aes(x = Buts, y = Equipe)) +
  geom_bar(stat = "identity", fill = "#F36143") +
  theme_minimal() +
  labs(title = "Nombre de buts marqués par équipe", x = "Nombre de buts", y = "Equipe")+
  theme(axis.text.y = element_text(angle = 0, vjust = 0.5, hjust = 1)) +
  coord_cartesian(xlim = c(75,140))

ggplot(big5_data_by_team, aes(x = Touches , y = Equipe)) + geom_bar(stat = "identity", fill = "#F36143") + 
  theme_minimal() + 
  labs(title = "Nombre de touches par équipe", x ="Nombre de touches" , y = "Equipe")+
  theme(axis.text.y = element_text(angle = 0, vjust = 0.5, hjust = 1)) +
  coord_cartesian(xlim = c(40000,52000))
...
### Exemple avec shiny Dashboard

Grâce à une application shiny, il est possible d'explorer plus facilement cette question pour tirer rapidement des conclusions sur l'importance ou non de certaines caractéristiques par rapport au classement d'une équipe. 

<p align="center">
  <img src="Graph/quest1/1.4.png" alt="Texte alternatif" style="display: block; margin: auto;">
</p>

Ici, vous pouvez voir un graphique similaire à ceux présentés précédemment, mais l'avantage de ce graphique dynamique est qu'il est possible de sélectionner les équipes de la Premier League que l'on souhaite afficher, elle se placeront par ordre de classement. Il est également possible de choisir quelle caractéristique nous pouvons afficher, comme par exemple ici, le nombre de touche.
...
Les graphiques montrent clairement des tendances. Nous constatons presque une relation linéaire entre le nombre de matchs et les blessures, à l'exception de la bundesliga.. Cela confirme l'idée que certains championnats peuvent être plus sujets aux blessures en raison de leur intensité de matchs.
