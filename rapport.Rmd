...
# Identifier les colonnes en double et les renommer
...
# Identifier les colonnes en double dans les données de goalkeeping et les renommer
...
# Filtrer les données pour Manchester City, Fulham et Southampton
...
ggplot(teams_of_interest, aes(x = Competition, y = Gls / MP, fill = Squad)) +
  ...
  labs(title = "Nombre moyen de buts par match pour Man City, Fulham et Southampton",
       x = "Compétition", y = "Nombre moyen de buts par match") +
...
ggplot(goalkeeping_teams_of_interest, aes(x = Competition, y = Arret, fill = Squad)) +
  ...
  labs(title = "Pourcentage d'arrêts pour Man City, Fulham et Southampton",
...
```{r plots6, echo=FALSE}
ggplot(goalkeeping_teams_of_interest, aes(x = Competition, y = `CS%`, fill = Squad)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = paste0(round(`CS%`, 2), " %")), position = position_dodge(width = 0.9), vjust = -0.5) +
  labs(title = "Pourcentage de clean sheets pour Man City, Fulham et Southampton",
...
<p align="center">
  <img src="PowerBi/Pbix.PNG" alt="Origines des joeurs de PL">
</p>
...
Pour approfondir cette analyse, il serait intéressant de considérer d'autres facteurs tels que l'état du terrain et le moment des matchs (partie de la saison). Ces éléments pourraient influencer les performances des équipes et offrir une compréhension plus complète des différences entre les compétitions de coupe et de championnat.
