...
```{r plot3}
# Sélectionner les 10 meilleures et 10 pires équipes en termes de possession
top_10 <- data %>%
  filter(Team_or_Opponent == "team") %>%
  arrange(desc(Poss)) %>%
  head(10)
...
champions_data <- champions_data %>%
  mutate(Possession = big5_possession$Poss[match(champions_data$Squad, big5_possession$Squad)])
